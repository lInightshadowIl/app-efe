...
// ✅ CAMBIO: App shell (HTML, JS, CSS) → Stale-While-Revalidate
// Sirve el caché AL INSTANTE, actualiza en background para la próxima visita.
// El sistema de versiones ya se encarga de notificar cuando hay datos nuevos.
if (
...
// Iconos y assets estáticos: Cache First (no cambian frecuentemente)
...
// ✅ NUEVA: Stale-While-Revalidate
// Responde con caché al instante (0ms de espera para el usuario).
// En background, actualiza el caché para que la próxima visita tenga la versión más reciente.
// Si no hay caché aún (primera visita), espera la red normalmente.
async function staleWhileRevalidate(request) {
const cache = await caches.open(CACHE_VERSION);
const cached = await cache.match(request);

```
// Lanza actualización en background sin bloquear (sin await)
...
// Si hay caché → responde YA, la red actualiza en segundo plano
// Si no hay caché (primera visita) → espera la red
return cached ?? await networkUpdate;
```

}

// Sin cambios: Network First con timeout de 6s (para version.json y horarios.json)
...
// Sin cambios: Cache First (para iconos y assets estáticos)
...
// SINCRONIZACIÓN EN SEGUNDO PLANO (sin cambios)
...
// MENSAJES DESDE LA APP (sin cambios)
